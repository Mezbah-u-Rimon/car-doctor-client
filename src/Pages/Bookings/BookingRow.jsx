


const BookingRow = ({ booking, handleDelete, handleConfirm }) => {
    const { _id, customerName, email, date, price, service, img, status } = booking;



    return (
        <tr>
            <th>
                <button onClick={() => handleDelete(_id)} className="btn btn-circle btn-sm hover:text-black bg-black text-white">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
            </th>
            <td>

                <div className="avatar">
                    <div className="w-24 rounded  h-24 mr-5">
                        <img src={img} />
                    </div>
                    <div>
                        <div className="font-bold">{service}</div>
                        <div className="text-base font-bold">
                            {date}
                        </div>
                    </div>
                </div>
            </td>
            <td>
                {customerName}
                <br />
                {email}
            </td>

            <td>
                $ {price}
            </td>


            <th>{status === 'confirm' ? <span> <button className="font-bold"> Confirmed </button></span> :
                <button onClick={() => handleConfirm(_id)} className="btn btn-ghost btn-xs"> Please Confirm </button>
            }
            </th>
        </tr >
    );
};

export default BookingRow;